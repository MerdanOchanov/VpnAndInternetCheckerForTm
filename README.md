...
- **Проверка доступности сайтов — по факту:** для не‑прокси режимов доступность
  ресурса определяется настоящим HTTPS‑запросом. Поэтому если DNS заворачивает сайт на
  `127.0.0.1`, но локальный редиректор/прокси реально его туннелирует (частый случай в TM),
  сайт корректно считается **доступным**; а «мёртвый» блок (ничего не отвечает) — недоступным.
