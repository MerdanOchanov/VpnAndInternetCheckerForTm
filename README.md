...
| 🟢 **Интернет через VPN** | открываются ли популярные сайты | `youtube.com`, `instagram.com`, `tiktok.com` через туннель, прокси или напрямую |
...
| `vpnTargets` | `youtube.com,instagram.com,tiktok.com` | ресурсы для проверки через VPN |
...
- **Проверка доступности сайтов — по факту, как в браузере:** доступность определяется
  настоящим HTTPS‑запросом через системные настройки прокси/PAC. Если это не проходит —
  ресурс резолвится через **DoH** (шифрованный DNS, который провайдер не может подменить),
  и делается TLS‑подключение к реальному IP. Так проверка совпадает с поведением Chrome
  (Secure DNS) и корректно определяет доступность, когда блокировка сделана подменой DNS,
  а трафик идёт через общий шлюз/VPN (например, Kerio) или локальный редиректор.
